...
kubectl apply -f k8s-manifests/config/storage/wait-for-consumer-storage.yaml
kubectl apply -f k8s-manifests/secrets/app-config-secret.yaml
kubectl apply -f k8s-manifests/config/rbac/storage-provisioner-fix.yaml
kubectl apply -f k8s-manifests/config/common/service-common-config.yaml
...
kubectl wait --for=condition=ready pod/mongo-2-0 --timeout=300s
kubectl wait --for=condition=ready pod/mongo-3-0 --timeout=300s
kubectl apply -f k8s-manifests/infrastructure/mongodb/replica-1/

echo "Waiting for infrastructure to be ready..."
kubectl wait --for=condition=ready pod/user-service-postgres-0 --timeout=120s
kubectl wait --for=condition=ready pod/auth-service-postgres-0 --timeout=120s
kubectl wait --for=condition=ready pod/order-service-postgres-0 --timeout=120s
kubectl wait --for=condition=ready pod/redis-0 --timeout=60s
kubectl wait --for=condition=ready pod/kafka-0 --timeout=300s
kubectl wait --for=condition=ready pod/mongo-1-0 --timeout=300s
...
kubectl wait --for=condition=ready pod -l app=config-server --timeout=180s

echo "Deploying UserService..."
docker build -t user-service:latest ./user-service
kubectl apply -f k8s-manifests/services/user-service/
kubectl wait --for=condition=ready pod -l app=user-service --timeout=240s

echo "Deploying AuthService..."
docker build -t auth-service:latest ./auth-service
kubectl apply -f k8s-manifests/services/auth-service/
kubectl wait --for=condition=ready pod -l app=auth-service --timeout=240s
...
kubectl wait --for=condition=ready pod -l app=api-gateway --timeout=240s

echo "Deploying PaymentService..."
docker build -t payment-service:latest ./payment-service
kubectl apply -f k8s-manifests/services/payment-service/
kubectl wait --for=condition=ready pod -l app=payment-service --timeout=240s
